...
你可以不中断screen窗口中程序的运行而暂时断开（detach）screen会话，并在随后时间重新连接（attach）该会话，重新控制各窗口中运行的程序。例如，我们打开一个screen窗口编辑/tmp/changeworld.txt文件：

```
screen vi /tmp/changeworld.txt
```
...
---
以上文章转载自博客园David Camp的博客

链接地址： http://www.cnblogs.com/mchina/archive/2013/01/30/2880680.html
